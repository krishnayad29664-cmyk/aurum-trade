...
export async function GET(request: NextRequest) {
  ...
  try {
    if (kind === 'candles') {
      ...
      if (!['1m','3m','5m','15m','1H'].includes(bar)) return NextResponse.json({ error: 'Unsupported interval' }, { status: 400 })
      const rows = await okx(`market/candles?instId=XAU-USDT-SWAP&bar=${bar}&limit=300`)
      const candles: Candle[] = rows.map((x: string[]) => ({ time: num(x[0]) / 1000, open: num(x[1]), high: num(x[2]), low: num(x[3]), close: num(x[4]), volume: num(x[6]), closed: x[8] === '1' })).reverse()
      if (candles.some((x, i) => x.low > Math.min(x.open,x.close) || x.high < Math.max(x.open,x.close) || (i > 0 && x.time <= candles[i-1].time))) throw new Error('Invalid candle sequence')
